...
class MultiIOProgramDecoder(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim,
        ...
        if masked_actions is not None:
            self.masked_actions = list(map(int, masked_actions.split()))
    ...
    def forward(self, tgt_inp_sequences, io_embeddings,
        ...
        '''
        #print('Before:', decoder_logit)
        # Action Masking
        action_mask = decoder_logit.data.new(decoder_logit.size()).fill_(0)
        for masked_action in self.masked_actions:
            action_mask[:, :, masked_action] = -float('inf')
        action_mask = Variable(action_mask, requires_grad=False)
        decoder_logit = decoder_logit + action_mask    
        #print('After:', decoder_logit)
        '''
