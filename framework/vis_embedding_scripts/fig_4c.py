...
EMBEDDING_NET_PATH = 'vis_embedding_scripts/embedding_networks/embedding_model_withoutNormConstraints_MultiKeyNavAB_dim3.pt'
embedding_dim = 3

def main():
    model_data = torch.load(EMBEDDING_NET_PATH, map_location=torch.device('cpu'))
    embedding_model = EmbeddingNetwork("MultiKeyNavAB", embedding_dim)
    ...
    plt.annotate('(Pick A)', (8.1, 3.03), c=('maroon'))
    plt.annotate('(Pick B)', (3.62, 5.6), c=('maroon'))
    plt.annotate('(Pick A, B)', (5.7, 4.2), c=('maroon'))
    plt.annotate('(Navigation)', (3.9, 1.2), c=('maroon'))
