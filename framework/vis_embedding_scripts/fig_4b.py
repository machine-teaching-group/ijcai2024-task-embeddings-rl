...
EMBEDDING_NET_PATH = 'vis_embedding_scripts/embedding_networks/embedding_model_withoutNormConstraints_MultiKeyNav_maskAllKeys_dim1.pt'
embedding_dim = 1
...
def main():
    ...
    for i, (x, color, marker) in enumerate(tasks):
        ...
        plt.scatter(embedding[0], 0, c=(color), marker=marker, alpha=0.4, edgecolors='black')

    plt.annotate('(Pick Keys)', (-3.8, 0.01), c=('maroon'))
    plt.annotate('(Navigation)', (+0.9, 0.01), c=('maroon'))
