...
def get_distance(s_1, s_2):
    f_1 = np.sum(np.asarray(s_1)[32:48]) - np.sum(np.asarray(s_1)[16:32])
    f_2 = np.sum(np.asarray(s_2)[32:48]) - np.sum(np.asarray(s_2)[16:32])
...
def run_techniques(config):
    ...
    data_test = load_json(f'{net_dir}/{config.data_path}/{config.prefix}/{config.num_options}/data_test.json')
